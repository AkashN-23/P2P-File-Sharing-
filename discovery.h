...
// A simple structure to hold found peers
struct Peer {
    std::string ip;
    std::string hostname;
};

// --- RECEIVER SIDE: LISTEN FOR "WHO IS THERE?" ---
    ...
    while (running) {
        ...
        if (len > 0) {
            ...
            if (strcmp(buffer, "P2P_DISCOVERY_REQ") == 0) {
                // Someone is looking for us! Reply with "P2P_RESP:<hostname>"
                std::string reply = "P2P_RESP:";
                reply += hostname;
                
                sendto(sock, reply.c_str(), reply.length(), 0, (struct sockaddr*)&sender_addr, sender_len);
                std::cout << "\n[*] Discovery: Pinged by " << inet_ntoa(sender_addr.sin_addr) << std::endl;
            }
        }
    }
    close(sock);
...
// --- SENDER SIDE: SHOUT "WHO IS THERE?" ---
std::vector<Peer> scan_for_peers() {
    ...
    struct timeval tv;
    tv.tv_sec = 2; // Wait 2 seconds for replies
    tv.tv_usec = 0;
    ...
    std::cout << "[*] Scanning for peers (2s timeout)..." << std::endl;
    ...
    while (true) {
        int len = recvfrom(sock, buffer, 1024, 0, (struct sockaddr*)&sender_addr, &sender_len);
        if (len < 0) break; // Timeout reached
